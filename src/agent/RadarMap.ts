...
export default class RadarMap {
    ...
    public static generate(world: w.World, beliefs: Beliefs): RadarMap {
        ...
        // Make the cells just outside of the coverage range higher payoff so radars tesselate
        const margin = collections.create2D(world.width, world.height, false);
        world.entities.forEach(radar => {
            if (radar.type === w.ItemType.Radar) {
                for (const n of traverse.neighbours(radar.pos, world, w.RadarRange + 1)) {
                    if (!covered[n.y][n.x] && !margin[n.y][n.x]) {
                        margin[n.y][n.x] = true;

                        payoffs[n.y][n.x] += Params.RadarMarginPayoff;
                    }
                }
            }
        });
